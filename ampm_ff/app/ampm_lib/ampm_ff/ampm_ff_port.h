...
#define AMPM_SEEK_END 0

AMPM_FIL* ampm_fopen(const char * filename, const char * mode);
AMPM_FRESULT  ampm_funlink(char *fileName);
#define  ampm_fclose ampm_f_close
...
#define ampm_ftell(fp) ((fp)->file_offset)
#define ampm_fsize(fp) ((fp)->file_size)
